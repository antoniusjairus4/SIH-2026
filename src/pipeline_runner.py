...
class TrackingSystemPipeline:
    """
    Unified Master Supervisor interconnecting Network, Perception, Estimation, and Metrics.
    """
    ...
    def process_single_frame(
        ...
        """
        Executes a single frame through the integrated perception -> estimation pipeline.
        ...
        3. Returns: TelemetryRecord contract for telemetry & metric logging.
        """
    ...
    def run_live_stream_loop(
        ...
        try:
            while self._running and (time.time() - start_time < duration_seconds):
                ...
                # 1. Process frame through Perception -> Estimation pipeline
                ...
                # 3. If control callback provided, compute & send PTZ command back to Unity
                if control_callback:
